...
from .mappings import reduce_mean, gather_from_col_tensor_model_parallel_region, scatter_to_tensor_model_parallel_region, TeleParallelRMSNormFunction
...
__all__ = [
    ...
    "TeleParallelRMSNorm",
    "TeleParallelRMSNormBeta",
    "TeleParallelRMSNormAlpha",
]
...
class TeleParallelRMSNorm(nn.Module):
    def __init__(self, dim, eps=1e-5, gather_output=False):
        super().__init__()
        self.eps = eps
    ...
    def norm(self, x):
        ...
        mean_x_squared = reduce_mean(mean_x_squared_local)
    ...
    def forward(self, x):
        ...
        if self.gather_output:  
            output_local = normalized_x * self.weight
            output = gather_from_col_tensor_model_parallel_region(output_local)
        else:
            output = normalized_x * self.weight
        return output

class TeleParallelRMSNormBeta(nn.Module):
    def __init__(self, dim, eps=1e-5, gather_output=False):
        super().__init__()
        self.eps = eps
    ...
    def forward(self, x_local):
        ...
        normalized_x = normalized_x.to(dtype)
        output_local = normalized_x * self.weight
        output = scatter_to_tensor_model_parallel_region(output_local)
        return output
    

class TeleParallelRMSNormAlpha(nn.Module):
    ...
    def forward(self, x):
        return TeleParallelRMSNormFunction.apply(x, self.weight, self.eps)

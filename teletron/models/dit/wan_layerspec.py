...
class WanDiTLayer(TransformerLayer):
    ...
    def __init__(
        ...
        self.norm2 = FP32LayerNorm(hidden_size, eps, elementwise_affine=True) if cross_attn_norm else nn.Identity()
    ...
    def forward(
        ...
        attn_output = self.self_attention(hidden_states=norm_hidden_states, 
                                          rotary_pos_emb=rotary_emb,
                                          attention_mask=None)
        hidden_states = (hidden_states.float() + attn_output * gate_msa).type_as(hidden_states)
        # 2. Cross-attention
        norm_hidden_states = self.norm2(hidden_states.float()).type_as(hidden_states)
        attn_output = self.cross_attention(hidden_states=norm_hidden_states, 

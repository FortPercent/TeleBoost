...
class WanVideoTransformer3DModel(VisionModule):
    def __init__(self, wan_config: WanParams, config: TransformerConfig):
        ...
        args = get_args()
        self.numer_layers = args.num_layers
        ...
        self.patch_embedding = nn.Conv3d(self.in_channels, self.inner_dim, kernel_size=self.patch_size, stride=self.patch_size)
        ...
        self.norm_out = FP32LayerNorm(self.inner_dim, self.eps, elementwise_affine=False)
    ...
    def forward(
        ...
        shift, scale = (self.scale_shift_table.to(t.device) + t.unsqueeze(1)).chunk(2, dim=1)

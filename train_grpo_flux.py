...
def compute_color_diversity_reward(images):
    """
    计算颜色多样性奖励 - 奖励颜色较少的图像
    
    Args:
        images: torch.Tensor, shape (B, C, H, W), 范围 [-1, 1]
    ...
    for i in range(batch_size):
        ...
        if image.shape[0] == 3:  # RGB
            ...
            # 转换为 HSV 颜色空间来更好地分析颜色
            image_np = image_small.permute(1, 2, 0).cpu().numpy()
            ...
            if high_saturation_mask.sum() > 0:
                ...
                final_reward = color_reward - diversity_penalty
            else:
                # 如果图像主要是灰色，给予中等奖励
                final_reward = 5.0
            
            # 额外检查：如果图像主要是单一颜色，给予额外奖励
            # 计算主导颜色的比例
            if len(hist) > 0:  # 修改：检查hist是否为空
...
def compute_simple_color_reward(images):
    """
    更简单的颜色奖励函数 - 基于颜色通道的标准差
    ...
    for i in range(batch_size):
        ...
        if image.shape[0] == 3:  # RGB
            ...
            # 计算RGB三个通道之间的差异
            color_variance = torch.var(torch.stack([r_mean, g_mean, b_mean]))
            
            # 计算图像整体的颜色标准差
            pixel_std = torch.std(image.view(3, -1), dim=1).mean()
            
            # 奖励函数：颜色变化越小，奖励越高
            variance_reward = max(0, 5.0 - color_variance.item() * 20)  # 通道间差异小的奖励
            std_reward = max(0, 3.0 - pixel_std.item() * 10)  # 像素标准差小的奖励
            
            final_reward = variance_reward + std_reward
        else:
            final_reward = 0.0
...
def run_sample_step(
    ...
    if grpo_sample:
        ...
        for i in progress_bar:  # Add progress bar
            ...
            # 这个flux step函数是核心采样步骤
            # 输入是模型的预测输出和当前潜在状态z，随机强度eta，噪声sigma，index i是干嘛的
...
def sample_reference_model( #参考模型采样
    args,
    device, 
    transformer,
    vae,
    ...
    if args.init_same_noise:
        input_latents = torch.randn(
                (1, IN_CHANNELS, latent_h, latent_w),  #（bs,c,h,w)
                device=device,
                dtype=torch.bfloat16,
            )
        print("初始化的噪声shape ", input_latents.shape)
    ...
    for index, batch_idx in enumerate(batch_indices):
        ...
        if not args.init_same_noise:
            input_latents = torch.randn(
                    (len(batch_idx), IN_CHANNELS, latent_h, latent_w),  #(bs,c,h,w)
                    device=device,
                    dtype=torch.bfloat16,
                )
            print("初始化的噪声shape ", input_latents.shape) #1,16,64,64
        input_latents_new = pack_latents(input_latents, len(batch_idx), IN_CHANNELS, latent_h, latent_w)
        print("打包后的噪声shape ", input_latents_new.shape) #1,1024,64
        image_ids = prepare_latent_image_ids(len(batch_idx), latent_h // 2, latent_w // 2, device, torch.bfloat16)
        grpo_sample=True
        progress_bar = tqdm(range(0, sample_steps), desc="Sampling Progress") #采样步数多少
        with torch.no_grad():
            ...
            print("flux latent: ", latents.shape) #bs,1024,64
            print("batch latents: ", batch_latents.shape)  #bs,9,1024,64
            print("batch log probs: ", batch_log_probs.shape) #bs,8
        ...
        #     # 使用简单的颜色奖励
        #     color_reward = compute_simple_color_reward(image)
        # else:
        #     # 使用复杂的颜色多样性奖励
        #     color_reward = compute_color_diversity_reward(image)
...
def train_one_step(
    ...
    #grpo需要 t时刻的latents和t+1时刻的latents,以及转移的log概率
    # print(all_latents.shape, all_latents[:, :-1][:, :-1].shape, all_latents[:, 1:][:, :-1].shape,all_log_probs.shape, all_log_probs[:, :-1].shape)
    #12,9,1024,64; 12,7,1024,64; 12,7,1024,64; 12,8; 12,7
    samples = {
    ...
    if dist.get_rank()==0:
        ...
        with open('./color_reward.txt', 'a') as f: 
            f.write(f"{gathered_reward.mean().item()}\n")
...
def main(args):
    ...
    # 移除HPSv2相关代码，设置虚拟的奖励模型和预处理器
    # reward_model = None  # 不再使用
    # processor = None     # 不再使用
    # preprocess_val = None # 不再使用

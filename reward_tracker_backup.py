...
class RewardTracker:
    ...
    def add_data(self, step, loss=None, reward_tensor=None, grad_norm=None):
        """
...
Data Points: {len(self.rewards)} individual rewards
    ...
    def _save_data_to_file(self, steps, losses, mean_rewards, std_rewards, grad_norms):
        ...
        try:
            ...
            # 保存最新数据
            latest_csv = os.path.join(self.save_dir, 'realtime_training_data.csv')
            df.to_csv(latest_csv, index=False)
            
        except Exception as e:
            # 如果pandas不可用，使用原生方法保存
            try:
                csv_path = os.path.join(self.save_dir, 'realtime_training_data.txt')
                with open(csv_path, 'w') as f:
                    f.write("step,loss,mean_reward,std_reward,grad_norm\n")
                    for i in range(len(steps)):
                        f.write(f"{steps[i]},{losses[i]:.6f},{mean_rewards[i]:.6f},{std_rewards[i]:.6f},{grad_norms[i]:.6f}\n")

...
from teletron.models.causwan.utils.fm_solvers import FlowDPMSolverMultistepScheduler, get_sampling_sigmas, retrieve_timesteps
from teletron.models.causwan.utils.fm_solvers_unipc import FlowUniPCMultistepScheduler
from teletron.models.causwan.modules.wan_wrapper import WanDiffusionWrapper, WanTextEncoder, WanVAEWrapper, WanFPSWrapper
...
class CausalFPSInferencePipeline(torch.nn.Module):
    def __init__(
        ...
        self.generator_uncondition = WanFPSWrapper(
            **getattr(args, "model_kwargs", {}), is_causal=True) if generator is None else generator
        for module in [self.generator_uncondition.model]:
            for param in module.parameters():
                param.requires_grad = False
        self.generator_uncond = self.generator_uncondition.to(self.device_uncond)

        self.generator_cond.model.num_frame_per_block = 1
        self.generator_uncond.model.num_frame_per_block = 1
    ...
    def inference(
        ...
        """
        #print text prompts
        print(f"[❗Trace] prompts({len(text_prompts)}): {text_prompts} | noise.shape={noise.shape}")
        ...
        ''' for next-block generation '''
        # clean_steps = [0, 0, 0, 1, 1, 1, 2, 2, 2, 3, 3, 3, 4, 4, 4, 5, 5, 5, 6, 6, 6]
        # all_num_frames = [3, 3, 3, 3, 3, 3, 3]
        # result = [[i for i, v in enumerate(clean_steps) if v == target] for target in range(7)]
        
        ''' for next-fps generation '''
        # clean_steps = [0, 0, 0, 0, 0, 1, 4, 3, 4, 2, 4, 3, 4, 1, 4, 3, 4, 2, 4, 3, 1]
        # clean_steps = [0, 0, 0, 0, 1, 3, 3, 3, 2, 4, 4, 4, 1, 5, 5, 5, 2, 6, 6, 6, 1]
        # all_num_frames = [4, 3, 2, 3, 3, 3, 3]

        ''' for anchor next-chunk generation ''' 
        # clean_steps = [0, 0, 1, 2, 2, 2, 2, 3, 3, 3, 3, 1, 4, 4, 4, 4, 5, 5, 5, 5, 1]
        # all_num_frames = [2, 3, 4, 4, 4, 4]
        # result = [[i for i, v in enumerate(clean_steps) if v == target] for target in range(6)]

        # clean_steps = [0, 0, 1, 2, 2, 3, 3, 4, 3, 2, 1, 1, 2, 3, 4, 3, 3, 2, 2, 1, 1]
        # all_num_frames = [2, 5, 6, 6, 2]
        ...
        # all_num_frames = [1, 2, 6, 3, 3, 3, 3]
        # result = [[i for i, v in enumerate(clean_steps) if v == target] for target in range(7)]

        # clean_steps = [0, 2, 2, 2, 2, 2, 2, 1, 1, 3, 3, 3, 4, 4, 4, 5, 5, 5, 6, 6, 6]
        # all_num_frames = [1, 2, 6, 3, 3, 3, 3]
        # all_num_frames = [1, 2, 6]
        # result = [[i for i, v in enumerate(clean_steps) if v == target] for target in range(7)]

        # clean_steps = [0, 2, 2, 2, 2, 2, 2, 2, 2, 2, 1, 3, 3, 3, 3, 3, 3, 3, 3, 3, 1]
        # all_num_frames = [1, 2, 9, 9]
        ...
        # import pdb;pdb.set_trace()
        #print current num frames and all num frames
        for current_num_frames in all_num_frames:
            print(f"[❗Trace] current_num_frames={current_num_frames}, all_num_frames={all_num_frames}")
            ...
            if (initial_latent is None) or (global_chunk_index != 0):
                ...
                for _, t in enumerate(tqdm(sample_scheduler.timesteps)):
                    ...
                    with torch.no_grad():
                        ...
                        flow_pred_uncond, _ = self.generator_uncond(

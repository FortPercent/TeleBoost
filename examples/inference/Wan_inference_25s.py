...
from teletron.utils.dataset import TextDataset, TextImagePairDataset
...
for i, batch_data in tqdm(enumerate(dataloader), disable=(local_rank != 0)):
    ...
    # -----Generate long(81 * num_rollout) frames-----
    num_rollout = 5
    ...
    for rollout_index in range(num_rollout):
        ...
        #initial_latent = latents[:, :num_overlap_frames] # test for code
        ...
        del sampled_noise, latents, video
        pipeline.vae = pipeline.vae.to("cpu")
        torch.cuda.empty_cache()

        sampled_noise = torch.randn(
            [args.num_samples, 21 - initial_num, 16, 60, 104], device="cuda", dtype=torch.bfloat16
        )

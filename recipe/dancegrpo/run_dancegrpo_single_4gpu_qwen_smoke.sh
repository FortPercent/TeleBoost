#!/usr/bin/env bash
# =============================================================================
# 4×H800 80GB smoke test (14B Wan2.2 + Qwen-VL-7B reward)
#
# Derived from run_dancegrpo_single_4gpu_smoke.sh + 1p3B_qwen variant, swapping reward_model from HPSv2 to
# Qwen2.5-VL-7B-Instruct (the production qwen.sh uses 32B; 4 GPUs cannot host two 32B at once):
...
exp_name=${project_name}_SMOKE_4gpu_qwen_${TIMESTAMP}
...
CKPTS_DIR=${CKPTS_DIR:-"/tmp/dancegrpo_smoke_qwen_ckpt"}
...
export TENSORBOARD_DIR=/tmp/dancegrpo_smoke_qwen_tb/${exp_name}

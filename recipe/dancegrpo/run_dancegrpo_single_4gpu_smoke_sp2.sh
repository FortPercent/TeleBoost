#!/usr/bin/env bash
# =============================================================================
# 4×H800 80GB smoke test (sp_size=2) — derived from run_dancegrpo_single.sh.
#
# Goal: on 4 H800 GPUs, run 14B Wan2.2 + HPSv2 reward for 1-2 steps to verify
#       1) Ray init + Hydra config resolve succeed
#       2) actor + ref + rollout + reward complete one forward+backward without OOM
#       3) Wan Ulysses SP > 1 patches (apply_wan_ulysses_patches) work end-to-end
# Not for training quality — do not interpret the reward curve.
#
# Key differences vs the production single.sh:
#   - n_gpus_per_node=4, sp_size=2   (was 8, 1)
...
exp_name=${project_name}_SMOKE_4gpu_sp2_${TIMESTAMP}
...
CKPTS_DIR=${CKPTS_DIR:-"/tmp/dancegrpo_smoke_sp2_ckpt"}
...
export TENSORBOARD_DIR=/tmp/dancegrpo_smoke_sp2_tb/${exp_name}

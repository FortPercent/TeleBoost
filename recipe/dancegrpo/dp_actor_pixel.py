...
from .dp_actor import DiffusionDataParallelPPOActor, logger

__all__ = ["DiffusionDataParallelPPOActorPixel"]


class DiffusionDataParallelPPOActorPixel(DiffusionDataParallelPPOActor):
    @GPUMemoryLogger(role="dp actor pixel", logger=logger)
    def update_policy(self, data: DataProto):
        ...
        for batch_idx, mini_batch in enumerate(dataloader):
            ...
            for step_idx in range(train_timesteps):
                ...
                if new_log_probs_step.dim() == old_log_probs_step.dim() - 1:
                    new_log_probs_step = new_log_probs_step.unsqueeze(0)

                if ratio_norm:
                    prev_sample_mean_old = batch_on_device.batch["prev_sample_mean"][:, step_idx]
                    if prev_sample_mean.dim() == prev_sample_mean_old.dim() - 1:
                        prev_sample_mean = prev_sample_mean.unsqueeze(0)

                    diff_squared = (prev_sample_mean - prev_sample_mean_old).pow(2)
                    ratio_mean_bias = diff_squared.flatten(start_dim=1).mean(dim=1)

                    sqrt_dt_scalar = sqrt_dt.mean() if torch.is_tensor(sqrt_dt) and sqrt_dt.ndim > 0 else sqrt_dt
                    std_dev_t_scalar = std_dev_t.mean() if torch.is_tensor(std_dev_t) and std_dev_t.ndim > 0 else std_dev_t
                    ...
                    ratio_mean_bias = ratio_mean_bias.view(-1, 1, 1, 1)
                ...
                if ratio_norm:
                    ...
                    data_dict["actor/sqrt_dt"] = (
                        sqrt_dt_scalar if isinstance(sqrt_dt_scalar, float) else sqrt_dt_scalar.item()
                    )
    ...
    def grpo_wan_one_step(
        ...
        if pre_latents.dim() == 4:
            pre_latents = pre_latents.unsqueeze(0)

        if latents.shape[1] != 16:
            raise ValueError(f"Expected 16 channels, got {latents.shape[1]} channels")

        boundary = getattr(self.config, "wan22_boundary", 0.9)
        sigma = sigma_schedule[i] if sigma_schedule is not None else None
        sample_guide_scale = self._select_wan22_guide_scale_local(guide_scale, timesteps, sigma, boundary)

        with torch.autocast("cuda", dtype=torch.bfloat16):
            with torch.no_grad():
                pred_uncond = transformer(
                    x=[sample for sample in latents],
                    t=timesteps,
        ...
        if return_stats:
            _, _, log_prob, prev_sample_mean, std_dev_t, sqrt_dt = self.wan_step(
                model_output,
                latents.squeeze(0).to(torch.float32) if latents.shape[0] == 1 else latents.to(torch.float32),
                self.config.eta,
                sigma_schedule,
                i,
                prev_sample=pre_latents.squeeze(0) if pre_latents.shape[0] == 1 else pre_latents,
                grpo=True,
                sde_solver=True,
                return_stats=True,
            )
    ...
    def wan_step(
        ...
        if grpo:
            log_prob = (
                ...
                / (2 * (std_dev_t**2))
            ) - torch.log(std_dev_t + 1e-8) - torch.log(torch.sqrt(2 * torch.as_tensor(torch.pi, device=latents.device)))
            if log_prob.dim() == 4:
    ...
    @staticmethod
    def _select_wan22_guide_scale_local(guide_scale, t, sigma, boundary):
        from .dp_actor import _select_wan22_guide_scale

        return _select_wan22_guide_scale(guide_scale, t, sigma, boundary)

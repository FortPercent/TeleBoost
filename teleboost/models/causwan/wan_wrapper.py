...
from teleboost.models.flow_match import CausalFlowMatchScheduler
from teleboost.models.causwan.modules.tokenizers import HuggingfaceTokenizer
from teleboost.models.causwan.modules.model import WanModel, RegisterTokens, GanAttentionBlock
from teleboost.models.causwan.modules.vae import _video_vae
from teleboost.models.causwan.modules.t5 import umt5_xxl
from teleboost.models.causwan.modules.causal_model import CausalWanModel
...
class WanTextEncoder(torch.nn.Module):
    def __init__(self, weights_path: str, tokenizer_path: str) -> None:
        """
        Args:
            weights_path: path to ``models_t5_umt5-xxl-enc-bf16.pth``.
            tokenizer_path: directory containing the umt5-xxl tokenizer files.
        """
        super().__init__()
...
class WanVAEWrapper(torch.nn.Module):
    def __init__(self, weights_path: str):
        """
        Args:
            weights_path: path to ``Wan2.1_VAE.pth``.
        """
        super().__init__()
        mean = [
...
class WanDiffusionWrapper(torch.nn.Module):
    def __init__(
        ...
        """
        Args:
            pretrained_path: HuggingFace-style model directory passed to ``from_pretrained``
                (e.g. a checkout of modelscope's Wan2.1-T2V-1.3B).
        """
        super().__init__()
        if is_causal:
            self.model = CausalWanModel.from_pretrained(
                pretrained_path, local_attn_size=local_attn_size, sink_size=sink_size)
        else:
